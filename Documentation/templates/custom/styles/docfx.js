...
$(function () {
  ...
  function buildVersionSelector()
  {
    ...
    return "<select name=\"version-selector\" id=\"version-selector\" onchange=\"selectVersion()\" value=\"\">" +
      `<option value='latest' ${currentVersion()=='latest' ? 'selected="selected"': ''}>Latest</option>` +
      versions.map((element) =>
      {
        if (element != "")
        {
          return `<option value=\"${element}\" ${currentVersion()==element ? 'selected="selected"': ''}>${element}</option>`
        }
